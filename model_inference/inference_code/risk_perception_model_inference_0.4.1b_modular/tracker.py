class ObjectTracker:
    ...
    def track_object(self, class_name, area_ratio):
        ...
        if self.tracked_objects[class_name]["alert_level"] == "관심":
            if (
                len(self.tracked_objects[class_name]["area_ratios"])
                >= self.frame_check_threshold
            ):
                recent_ratios = self.tracked_objects[class_name]["area_ratios"][
                    -self.frame_check_threshold :
                ]
                ...
                if ratio_change >= self.danger_ratio:
                    ...
                    print(
                        f"[위험] {class_name} detected with area ratio increase to {recent_ratios[-1]:.2f}"
                    )
                elif ratio_change >= self.warning_ratio:
                    self.tracked_objects[class_name]["alert_level"] = "경고"
                    print(
                        f"[경고] {class_name} detected with area ratio increase to {recent_ratios[-1]:.2f}"
                    )

        if class_name in ["fire", "smoke"]:
            if (
                self.tracked_objects[class_name]["frames_since_first_detection"]
                > self.fire_smoke_frame_check_threshold
            ):
                print(
                    f"[경고] {class_name} detected for more than {self.fire_smoke_frame_check_threshold} frames"
                )
