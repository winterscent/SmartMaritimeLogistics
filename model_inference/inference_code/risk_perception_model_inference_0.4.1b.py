...
def process_image(image_path, model, conf_threshold=0.25):
    ...
    for track in tracks:
        ...
        if det_conf is not None:
            print(
                f"Detected: {class_name} with confidence {det_conf:.2f} at {image_path},"
...
def track_object(class_name, area_ratio):
    ...
    if tracked_objects[class_name]["alert_level"] == "관심":
        if len(tracked_objects[class_name]["area_ratios"]) >= frame_check_threshold:
            ...
            recent_ratios = tracked_objects[class_name]["area_ratios"][
    ...
    if class_name in ["fire", "smoke"]:  # 클래스 이름을 소문자로 통일
        if (
            tracked_objects[class_name]["frames_since_first_detection"]
            > fire_smoke_frame_check_threshold
        ):
            print(
                f"[경고] {class_name} detected for more than {fire_smoke_frame_check_threshold} frames"
